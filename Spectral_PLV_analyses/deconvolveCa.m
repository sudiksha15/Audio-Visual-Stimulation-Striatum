...
switch lower(options.method)
    ...
    case 'thresholded'  %% Use hard-shrinkage method
        ...
        elseif strcmpi(options.type, 'exp2')   % difference of two exponential functions
            ...
            timed=1000/20:1000/20:win*50;
C1=[ 0  0  +0 -1000  1 ];
C2=[ 0  0  +0 -300  1 ];
[prob2]= double_exp(timed,C1,C2,1);
prob=exp2kernel([d r]./50, win);
            options.pars = (double(prob2./max(prob2))').*2;%(exp(log(d)*(1:win)) - exp(log(r)*(1:win))) / (d-r); % convolution kernel
            [c, s] = onnls(y-b0, options.pars, options.lambda, ...
