...
figure,plot(freq2.freq,aCOHs)

neuron=aNeuron(73),
 [ s_oasis, onsets2 ,onsets]= deconvSudi(tracesF(:,neuron));
 
figure('COlor','w','Position', [ 300 400 500 180],'Renderer', 'painters')
plot(tracesF(:,neuron),'k','Linewidth',1)
hold on,plot(onsets2,ones(1,length(onsets2)),'.r','Markersize',10)
%hold on,plot(deltPhase)
hold on,plot(zscore(deltsignal))
axis tight


figure('COlor','w','Position', [ 300 400 200 180],'Renderer', 'painters')
;rose(deltPhase(onsets2))
...
axis tight
 ...
 plot(freq2.freq,nanmean(aCOHs(:,MOT_respALL==1),2),'r')
  %plot(freq2.freq,nanmean(aCOHs(:,MOT_respALL==0),2),'b')
axis tight

     figure('COlor','w','Position', [ 300 400 250 180],'Renderer', 'painters') 
 fill_error_area2(freq2.freq,nanmean(aCOHs(:,Stim_respALL==0),2), nanstd(aCOHs(:,Stim_respALL==0),[],2)./sqrt(size(aCOHs(:,Stim_respALL==0),2)),[ 0.5 0.5 0.5])
 ...
 plot(freq2.freq,nanmean(aCOHs(:,Stim_respALL==1),2),'r')
  plot(freq2.freq,nanmean(aCOHs(:,Stim_respALL==0),2),'b')
axis tight

figure,plot(freq2.freq,aCOHs(:,44)')
