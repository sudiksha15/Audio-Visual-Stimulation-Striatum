function image_projection_sudi(fileDir)
    ...
    if isempty(fileDir)
        ...
        filename_list = findNestedFiles('/mnt/eng_handata/eng_research_handata2/Florence_Liang/tfs/608452_mc_stim_40hz00001/motion_corrected/', '"*.tif"')
        
    else
        filename_list = {fileDir};
    end
    sprintf(filename_list{1})
    ...
    for filename_idx = 1:numel(filename_list)
        ...
        video_matrix = tiffs2matrix('m_', 1, 0);
        ...
        imagesc(final_image);
        axis image;
        saveas(fig_1,'full_projection_m_f_max_min_batch.fig');
        close all;
