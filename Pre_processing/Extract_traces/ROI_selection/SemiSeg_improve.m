...
function [CellList,outline] = SemiSeg_improve(imArray, inputCellList,outline)
%imArray=a; inputCellList=[];
    %Start up
    fullsz = size(imArray);
    if numel(fullsz) == 3
        nimages = fullsz(3);
    ...
    if ~isempty(inputCellList) %If given input CellList
        ...
        for idx = 1:numel(CellList)
            plot(outline(idx).XData,outline(idx).YData,'g')
        end
        end
        
        CellNum = numel(CellList);
    else
        CellList = struct([]);
    ...
    while (true)
        answer = questdlg('What to do next?', ...
	'Selection', ...
	'Add ROI','Remove ROI','End','End');
       % ,'Check for index'
        
            if strcmp(answer,'Add ROI')
    ...
    click_type = questdlg('Select Click Type for ROI Selection','Would you like just Circles or Full ROI Selection?','Circles','Full ROI','Break','Full ROI');
    switch click_type
        case 'Break'
            break
        case 'Circles' 
                ...
                for inputs=1:length(xList) %Loop through points
                    ...
                    outline(CellNum) = plot(b{1}(:,2),b{1}(:,1),'g'); %Plot for visualization
                    idx=idx+1;
        ...
        case 'Full ROI'
                ...
                switch processtype
                    case 'Raw' %Threshold from Raw Input Image (Only case that works if only single frame is inputted
                        ...
                        while(true)
                            ...
                            outline(CellNum) = plot(b{1}(:,2),b{1}(:,1),'g');
                            %Keep this ROI?
            ...
            elseif strcmp(answer,'Remove ROI')
        ...
        for step = 1:numel(CellList)
            temp = intersect(CellList(step).(pixel_list), rmpixels);    
            if ~isempty(temp)
                rmrois(pos) = step;
                pos = pos+1;
                delete(outline(step));
  ...
  outline(allRemoved)=[];
   ...
   elseif strcmp(answer,'Check for index')
