...
def motion_correction(filename_list=None, save_filename=None, save_foldername='motion_corrected', as_group = True, save_format='tif', ref_frame = None, **kwargs):
    ...
    for filename_idx, filename in enumerate(tqdm(filename_list, desc='Files')):
        ...
        if file_extension=='.tif':
            ...
            images = file_data['image_data']
        elif file_extension=='.dcimg':
            file_data = dcimgapi.Dcimg()
            file_data.open(filename)
            images = []
            for frame_idx in range(file_data.numberof_frame()):
                images.append(file_data.readframe(frame_idx))
        ...
        if ref_frame is not None:
            if filename_idx==0:
                ref_image = np.squeeze(images[ref_frame,:,:])
            ...
            if as_group:
                try:
                    ...
                    print("\n\033[1;32;40mGenerating reference with: {0}\033[0m".format(filename))
...
def apply_tv_denoise(images):
    ...
    # print("Applying histogram equalization....")
    for frame_idx, image in enumerate(images):
        images[frame_idx, :, :] = denoise_tv_chambolle(image,weight=1e-3,eps=1e-6)
    if squeeze_image:
        images = np.squeeze(images)
    return images
...
def calculate_shift(images, ref_image=None, local_subpixel=[1, 0], global_subpixel=1,
    ...
    elif process_functions=='gcamp':
        process_functions=['remove_edges', 'apply_highpass_flt', 'apply_sharpen', 'apply_tv_denoise', 'apply_std_eq']
    elif process_functions=='voltage':
        #process_functions=['remove_edges', 'apply_highpass_flt', 'apply_sharpen', 'remove_intensity', 'apply_std_eq']
        process_functions=['remove_edges', 'apply_highpass_flt', 'apply_sharpen'] # switch to this prameters on 2019/8/28
    else:
        pass
    ...
    process_function_list = {
        ...
        'apply_tv_denoise': apply_tv_denoise,
